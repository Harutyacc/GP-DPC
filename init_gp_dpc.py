...
class DensityPeakClustering:
    ...
    def _initialize_parameters(self, X):
        ...
        n_candidates = 32
        candidate_indices = np.argsort(gamma)[-n_candidates:]
        
        for n_components in sorted(self.n_components_candidates, reverse=True):
            selected_centers = [candidate_indices[-1]]
            
            # 贪心算法选择
            while len(selected_centers) < n_components:
          
                min_distances = np.min([self._cached_distances[i, selected_centers] 
                                     for i in candidate_indices], axis=1)
                
                remaining_indices = [i for i in range(len(candidate_indices)) 
                                  if candidate_indices[i] not in selected_centers]
                if not remaining_indices:
                    break
                    
                next_center_idx = remaining_indices[np.argmax(min_distances[remaining_indices])]
                selected_centers.append(candidate_indices[next_center_idx])
            
            centers_points = X[selected_centers]
            
            # 初始化协方差矩阵
            n_samples = X.shape[0]
            n_samples_per_component = n_samples // n_components
            ...
            covariance /= n_components
            min_covar = 1e-6
            covariance += np.eye(X.shape[1]) * min_covar
            covariance = (covariance + covariance.T) / 2
            
            # 使用KLD验证中心点
            has_redundancy, min_klds = self.verify_centers(centers_points, covariance)
            
            # 如果是QPSK（4个中心点）或没有冗余，接受当前结果
            if n_components == 4 or not has_redundancy:
