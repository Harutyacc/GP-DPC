...
modulation_regions = [
    (2, 8, 'QPSK', r'$K \in [2,8]$'),
    (8, 14, '8PSK', r'$K \in (8,14]$'),
    (14, 20.1, '16QAM', r'$K \in (14,20]$')
]

# 使用两条垂直虚线划分三个区域
for xmin, xmax, label, text in modulation_regions:
    plt.axvline(x=xmin, color='gray', linestyle='--', linewidth=2)
    plt.axvline(x=xmax, color='gray', linestyle='--', linewidth=2)
    # 在虚线中间添加文本
...
# 保存设置
plt.savefig('RMSE_with_vertical_lines.pdf', 
            dpi=300, 
            bbox_inches='tight')
